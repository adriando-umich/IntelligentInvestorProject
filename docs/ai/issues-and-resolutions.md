...
- Production can still show `No projects yet` for a real returning user when the latest successful sign-in lands on a different Supabase `auth.users.id` than the historical membership rows; the new same-email recovery path is written in source, but `20260411223000_relink_memberships_by_email.sql` still needs a live Supabase apply.
