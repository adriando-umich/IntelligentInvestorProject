...
- The new `project_member_activity` audit log plus members-page governance polish are implemented locally, but this session has not yet applied `20260328210000_project_member_activity.sql` or promoted a matching production deployment.
- The repo now has stricter deploy docs, but this workspace still needs an authenticated Vercel session before it can promote the pending `b05e02f` production release.
- The duplicate-member identity fix is implemented locally, but `20260328233000_canonical_project_member_identities.sql` still needs a live Supabase apply; this workspace does not currently have a Supabase access token configured.
